...
<body>
    ...
    foreach ($persona as $row) {
        ...
        $nombre = SED::decryption($row['nombre_repartidor']);
        $apellido = SED::decryption($row['apellido_repartidor']);
        $usename = SED::decryption($row['username_repartidor']);
        $celular = $row['celular_repartidor'];
        $edad_repartidor = SED::decryption($row['edad_repartidor']);
        $gener = SED::decryption($row['genero_repartidor']);
        $direccion = SED::decryption($row['direcci_repartidor']);
    ...
    <div class="container">
        <div class="container " style="background-color:orange; ">
            <form action="" method="post">
                ...
                <div class="row g-3">
                    <div class="col">
                        <label for="" style="color:white;" class="formulario__label"><strong>Edad: </strong></label>
                        <input type="text" class="form-control" name="edad" value="<?php echo $edad_repartidor; ?>">
                    </div>
                    <div class="col">
                ...
                <div class="row g-3">
                    <div class="col">
                        <label for="" style="color:white;" class="formulario__label"><strong>Dirección: </strong></label>
                        <input type="text" class="form-control" name="direccione" value="<?php echo $direccion; ?>">
                    </div>
                    <div class="col">
            ...
            if (isset($_POST['edit_infor'])) {
                ...
                $direecion_emp = $_POST['direccione'];
                $celular = "51" . $celu;
                $nombreus = sed::encryption($nombre);
                $apellidoe = sed::encryption($apellido);
                $user_nam = sed::encryption($usname);
                $edade = sed::encryption($edad_r);
                $generoe = sed::encryption($generous);
                $direccione = sed::encryption($direecion_emp);
                if (!empty($_POST['nombre']) && !empty($_POST['apellido']) && !empty($_POST['username']) && !empty($_POST['direccione']) && !empty($_POST['genero'])) {
                    if ($edad_r >= 17) {
                        ...
                        SET nombre_repartidor = '$nombreus', apellido_repartidor='$apellidoe', username_repartidor= '$user_nam', edad_repartidor= '$edade',
                        genero_repartidor= '$generoe', celular_repartidor= '$celu', direcci_repartidor= '$direccione', codciudad='$ciudaduse'
                        WHERE id_repartidor='$id_session'");
                            if (@$resul) {
                                echo "<script>location.href='admeditarepartidor.php'</script>";
                            } else {
                    ...
                    echo '<div class="alert alert-danger" role="alert">Te faltan datos qu completar. </div>';
                }
            }
            ?>
        </div><br>
