...
<body>
    ...
    foreach ($persona as $row) {
        ...
        $nombre = SED::decryption($row['nombreempresa']);
        $direccionempresa = SED::decryption($row['direccionempresa']);
        $usename = SED::decryption($row['username_empresa']);
    ...
    <div class="container">
        <div class="container " style="background-color:orange; ">
            <form action="" method="post">
                ...
                <div class="row g-3">
                    <div class="col">
                        ...
                        <label for="" style="color:white;" class="formulario__label"><strong>Empresa: </strong></label>
                ...
                <div class="row g-3">
                    <div class="col">
                        ...
                        <input type="text" class="form-control" name="direccion" value="<?php echo $direccionempresa; ?>">
                    </div>
                </div>
                <center>
            ...
            if (isset($_POST['edit_infor'])) {
                ...
                $nombre = $_POST['nombre'];
                $direcccion_empresa = $_POST['direccion'];
                $usname = $_POST['username'];
                $celu = $_POST['celular'];
                $ciudaduse = $_POST['ciudad'];
                $celular = "51" . $celu;
                $nombreus = sed::encryption($nombre);
                $direccione = sed::encryption($direcccion_empresa);
                $user_nam = sed::encryption($usname);
                if (!empty($_POST['nombre']) && !empty($_POST['direccion']) && !empty($_POST['username'])) {
                    if ($celular <= 51999999999 && $celular >= 51900000000) {
                        ...
                        SET nombreempresa = '$nombreus', celularempresa= '$celu', username_empresa= '$user_nam', direccionempresa='$direccione', codciudad='$ciudaduse'
                        WHERE logueo_empresa.id_empresa='$id_session'");
                        if (@$resul) {
                            echo "<script>location.href='admeditempresa.php'</script>";
                        } else {
